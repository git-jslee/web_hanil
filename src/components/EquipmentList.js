...
function EquipmentList({ click }) {
  ...
  useEffect(function () {
    axios
      .get('http://172.20.21.59:1337/equips')
      .then((result) => {
  ...
  const getEquipment = (click) => {
    ...
    if (click === 'ALL') {
      ...
      equipmentFilter = equipments.filter(
        (equipment) => equipment.equipment_category.name === click
      );
    }
  };
  ...
  const equipmentMap = equipmentFilter.map((equipment, index) => {
    return (
      <div key={index} className="slicklist img">
        <img
          src={API_URL + equipment.image.formats.small.url}
          alt={equipment.equipment_category.name}
        />
      </div>
    );
  });
  ...
  return (
    <>
      {equipments.length !== 0 ? (
        <Slider {...slickSettings}> {equipmentMap} </Slider>
      ) : (
        <h1>로딩중</h1>
      )}
    </>
